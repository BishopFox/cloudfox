...
func TestOutputSelector(t *testing.T) {
	...
	for _, s := range subTests {
		...
		t.Run(s.name, func(t *testing.T) {
			...
			OutputSelector(s.verbosity, s.outputType, header, body, s.outputDirectory, fmt.Sprintf("%s%s", s.callingModule, s.fileNameExtension), s.callingModule, s.prefixIdentifier)
		})
	}
	fmt.Println()
}
