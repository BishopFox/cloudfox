...
func (m *AccessTokensModule) writeOutput(logger internal.Logger) {
	...
	for _, c := range m.Credentials {
		if c.Type == "access_token" && c.Validity == "ACTIVE" && c.AccessToken != "" {
			exploitCmds.WriteString(fmt.Sprintf("# Account: %s (Profile: %s, Project: %s)\n", c.Account, c.Profile, c.Project))
			exploitCmds.WriteString(fmt.Sprintf("curl -s -H 'Authorization: Bearer %s' https://www.googleapis.com/oauth2/v1/tokeninfo\n", c.AccessToken))
			exploitCmds.WriteString(fmt.Sprintf("curl -s -H 'Authorization: Bearer %s' https://cloudresourcemanager.googleapis.com/v1/projects\n\n", c.AccessToken))
			hasExploitCmds = true
		}
		if c.Type == "sa_key" {
